...
\____/  \__, |  \_|     \__,_| \__| \___/ .
  ...
  .      |___/   SPAM BY PATO       
    ...
    contributors = "Ajudantes: "+" ".join(__CONTRIBUTORS__)
...
def get_phone_info():
    while True:
        ...
        if ((len(target) <= 6) or (len(target) >= 12)):
            mesgdcrt.WarningMessage(
                "O numero do telefone ({target})".format(target=target) +
                "que você digitou é inválido")
            continue
        return (cc, target)
...
def pretty_print(cc, target, success, failed):
    ...
    mesgdcrt.SectionMessage("O Atake está em andamento - seja paciente")
    mesgdcrt.GeneralMessage(
        "Fique conectado à internet durante o atake")
    mesgdcrt.GeneralMessage("alvo       : " + cc + " " + target)
    mesgdcrt.GeneralMessage("tentativas         : " + str(requested))
    mesgdcrt.GeneralMessage("envadas   : " + str(success))
    mesgdcrt.GeneralMessage("falhadas       : " + str(failed))
    mesgdcrt.WarningMessage(
        "By pato")
    mesgdcrt.SuccessMessage("Use com moderaçao")
...
def workernode(mode, cc, target, count, delay, max_threads):
    ...
    clr()
    mesgdcrt.SectionMessage("espere")
    mesgdcrt.GeneralMessage(
        "Fique conectado à internet durante o atake")
    mesgdcrt.GeneralMessage("API Version   : " + api.api_version)
    mesgdcrt.GeneralMessage("alvo        : " + cc + target)
    mesgdcrt.GeneralMessage("Quantidade        : " + str(count))
            ...
            for job in as_completed(jobs):
                ...
                if result is None:
                    mesgdcrt.FailureMessage(
                        "O limite de atake do seu alvo foi atingido")
    ...
    print("\n")
    mesgdcrt.SuccessMessage("stake completo!")
    time.sleep(1.5)
    bann_text()
    sys.exit()
